...
Object.assign(session, {
  ...
  _initRun: function (o) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      // 检测设备唯一识别号
      if (o.sessionData && o.sessionData.sessionCard) {
        return o
      } else {
        var userAgent = ''
        if (o.req && o.req.headers) {
          userAgent = o.req.headers['user-agent'] || o.req.headers['User-Agent'] || o.req.headers['userAgent'] || o.req.headers['useragent'] || userAgent
        }
        ...
        .then(sessionCard=>{
            ...
            o.sessionData.sessionCard = sessionCard
            return o
        })
      }
    }).then(function (o) {
      o.requestId = o.requestId || util.createRequestId()

      // 授权字符串
      var authorization = 'session-init-v1'
      authorization += '/' + o.requestId
      authorization += '/' + (o.sessionData.sessionId || '0')
      authorization += '/' + o.sessionData.sessionCard
      authorization += '/' + sign.getUTCServerTime(o.sessionData.differenceTime || 0) + '/' + '1800'
      var signingKey = sign.HmacSHA256(authorization, (o.sessionData.sessionKey || 'sessionKey'))
      // 生成加密key
      ...
      console.log('o', o)
      // 返回一个请求
      return request(api.baseUrl+o.path, '', o.method, o.headers)
      .then(function (res) {
      ...
      .then(function (res) {
        return new Promise(function (resolve, reject) {
          ...
          var sessionData = res.sessionData
          // 服务器时间
          ...
          if (sessionData.expiresTime !== undefined && sessionData.expiresTime !== null) {
  ...
  // 获取正确的会话数据
  getTrueData: function (requests, response, isClean) {
    return new Promise(function (resolve, reject) {
      var o = {}
      var isRun = false
      // 处理回调
    ...
    session.getData(sessionO).then(function (o) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        isSessionDataPass = isSessionDataPass && (!o.isSessionInit)
          // 基本需要的数据检测
        isSessionDataPass = isSessionDataPass && o.sessionData && o.sessionData.sessionId && o.sessionData.sessionKey && o.sessionData.sessionCard
          // 检测事件
        isSessionDataPass = isSessionDataPass && o.sessionData.expiresTime && util.time() < (o.sessionData.expiresTime - 5)
        // 为了保证没有问题，提前5秒钟过期
      ...
      if (sessionO.req && sessionO.req._getSessionTrueDataCbs) {
        while ((cbt = sessionO.req._getSessionTrueDataCbs.shift())) {
          if (cbt && cbt[0] && cbt[2] && util.isFunction(cbt[0])) {
            util.copyObjByKey(cbt[2], o, keys)
            cbt[0](cbt[2])
          }
          cbt = void 0
        }
