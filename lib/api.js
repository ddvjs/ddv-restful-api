...
function ddvRestFulApi (path, requests, response) {
  ...
  return modelPromise(function () {
    return Promise.resolve()
      ...
      .then(function (body) {
        if (this.isAuth === true) {
          return ddvRestFulApiNextRun(this.auth, body, this.requests, this.response, this.request, this.onAccessKey, 0, (api.onAccessKeyTrySum || 3))
        } else {
          return this.request(auth.getUri(), body, auth.method, auth.headers)
          .then(function (res) {
            auth = body = void 0
            return this.isNative ? res : parseDataByBody(res)
          })
        }
      }.bind(this))
      .then(function (res) {
...
// 运行这个请求
function ddvRestFulApiNextRun (auth, body, requests, response, requestRun, onAccessKey, tryNum, trySum) {
      ...
      return requestRun(auth.getUri(), body, auth.method, auth.headers)
        ...
        .catch(function (e) {
          ...
          if (parseInt(e.statusCode) === 403 && tryNum < trySum) {
            // 重新运行一次
            r = ddvRestFulApiNextRun(auth, body, requests, response, requestRun, onAccessKey, (tryNum + 1), trySum)
          } else {
            // 还是原路抛出错误
