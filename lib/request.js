...
function request (uri, body, method, headers) {
  method = method || 'GET'
  var useWs = this && (typeof this.isAvailable === 'function' && this.isAvailable()) && method.toLowerCase() === 'get'
  return isWindow
    ? useWs
      ? wsRequest.call(this, uri, body, method, headers)
      : ajaxbyWindow(uri, body, method, headers)
    : ajaxbyNode(uri, body, method, headers)
}
function wsRequest (uri, body, method, headers) {
  var cb = this.wsRequest(uri, body, method, headers)
