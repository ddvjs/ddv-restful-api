...
const Request = request.Request = class ddvRequest {
    ...
    _run(){
        ...
        sign(this).then(function(o){
            ...
            if (_this&&_this.callback) {
                if (_this.serverRes.statusCode >= '200'&&_this.serverRes.statusCode < '300') {
                    _this.callback(null, _this);
                }else{
                    var e = new Error(_this.serverRes.status)
    ...
    _runRequest(resolve, reject){
        ...
        //发送请求
        req = (opt.protocol==='http:'?http:https).request(opt, (response)=>{
            this.serverRes.body = '';
            response.on('data', (data)=>{
                this.serverRes.body += data;
            }).on('end',()=>{
                this.serverRes.statusCode = response.statusCode || 200;
                this.serverRes.status = response.statusMessage.toString() || 'UNKNOW_ERROR';
                this.serverRes.rawHeaders = response.rawHeaders || [];
                this.serverRes.headers = response.headers || Object.create(null);
                resolve(this);
            });
        });
        req.write(this.body);
