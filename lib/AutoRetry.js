...
Object.assign(AutoRetryPro, {
  ...
  autoRetry: function autoRetry (runPromise) {
    ...
    .then(function () {
      if (!util.isFunction(runPromise)) {
        ...
        return this.emitCatch(e)
      }
    }.bind(this))
    .then(function () {
      return this.autoRetryRun(runPromise)
      .then(function thenCb (res) {
        return this.emitThen(res, runPromise)
      }.bind(this), function catchCb (e) {
        return this.emitCatch(e, runPromise)
      }.bind(this))
    }.bind(this))
  },
  ...
  emitThen: function emitThen (res, runPromise, i) {
    ...
    if (Array.isArray(this.onThenEmits) && (fn = this.onThenEmits[i]) && util.isFunction(fn)) {
      return new Promise(function (resolve, reject) {
        ...
        return this.emitThen(res, runPromise, ++i)
  ...
  // 触发异常
  emitCatch: function emitCatch (e, runPromise, i) {
    var fn, fnRes
    i = i || 0
    if (Array.isArray(this.onCatchEmits) && (fn = this.onCatchEmits[i]) && util.isFunction(fn)) {
      return new Promise(function (resolve, reject) {
        try {
          ...
          }.bind(this), this.onCatchEmits.length)
        } catch (e) {
          reject(e)
        }
        if (fnRes && fnRes.then) {
        ...
        return this.emitCatch(e, runPromise, ++i)
      }.bind(this))
    } else {
      return Promise.reject(e)
    }
